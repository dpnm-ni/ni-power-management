...
def call_migrate(vnf_id, dst_node_id):
    ...
    while(True):
        ...
        if get_vnf_info(vnf_id).status == "VERIFY_RESIZE" :
            ...
            stdin, stdout, stderr = ctrl_ssh.exec_command(command, timeout=60)
            break
        elif get_vnf_info(vnf_id).status == "ACTIVE" :
            break
...
class Testbed(Api):
    ...
    def __init__(self,consolidation) -> None:
        ...
        self.vnfs_data = [vnf for vnf in vnfs_data if vnf.node_id in self.node_ids and vnf.id]
        ...
        for srv in self.srvs:
            for vnf in srv.vnfs:
                if vnf.id == vnf_id:                 
                    ...
                    #Real Consolidating!!
                    print("Consolidating VNF : {}, Server : {}".format(self.vnfs[vnf_id].oid, self.srvs[srv_id].oid))
                    call_migrate(self.vnfs[vnf_id].oid, self.srvs[srv_id].oid)
